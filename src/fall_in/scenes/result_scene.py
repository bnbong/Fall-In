...
class ResultScene(Scene):
    ...
    def render(self, screen: pygame.Surface) -> None:
        ...
        # Title
        title = title_font.render(f"라운드 {self.round_number} 정산", True, AIR_FORCE_BLUE)
        ...
        # Divider
        pygame.draw.line(screen, AIR_FORCE_BLUE, (100, start_y + 35), (SCREEN_WIDTH - 100, start_y + 35), 2)
        ...
        for player in self.players:
            ...
            pygame.draw.rect(screen, (200, 200, 200), (col_x[2], row_y + 12, gauge_width, gauge_height), border_radius=3)
            if fill_ratio > 0:
                pygame.draw.rect(screen, fill_color, (col_x[2], row_y + 12, int(gauge_width * fill_ratio), gauge_height), border_radius=3)
            pygame.draw.rect(screen, AIR_FORCE_BLUE, (col_x[2], row_y + 12, gauge_width, gauge_height), width=1, border_radius=3)
            
            total_text = small_font.render(f"{total}/{GAME_OVER_SCORE}", True, AIR_FORCE_BLUE)
        ...
        # Hint
        hint_text = small_font.render("[SPACE] 또는 버튼 클릭으로 계속", True, AIR_FORCE_BLUE)
        screen.blit(hint_text, (SCREEN_WIDTH // 2 - hint_text.get_width() // 2, SCREEN_HEIGHT - 30))
