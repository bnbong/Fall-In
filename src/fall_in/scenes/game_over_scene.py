...
from fall_in.config import (
    ...
    AIR_FORCE_BLUE, WHITE, LIGHT_BLUE,
    DANGER_SAFE, DANGER_DANGER
)
...
class GameOverScene(Scene):
    ...
    def __init__(
        ...
        # Find human player
        self.human_player = next(p for p in players if p.player_type == PlayerType.HUMAN)
    ...
    def render(self, screen: pygame.Surface) -> None:
        ...
        round_text = font.render(f"진행 라운드: {self.round_number}", True, AIR_FORCE_BLUE)
        screen.blit(round_text, (stats_rect.x + 30, stats_y))
        
        score_text = font.render(f"최종 위험도: {self.human_player.penalty_score}", True, AIR_FORCE_BLUE)
        ...
        total_text = small_font.render(f"총 보유 수당: {total_currency} 원", True, AIR_FORCE_BLUE)
        ...
        screen.blit(rankings_title, (SCREEN_WIDTH // 2 - rankings_title.get_width() // 2, rankings_y))
