...
from fall_in.config import AIR_FORCE_BLUE
...
class Commander:
    ...
    def _draw_placeholder(self, screen: pygame.Surface) -> None:
        ...
        pygame.draw.rect(
            screen, (50, 80, 120), (self.x - 30, head_y - 50, 60, 10), border_radius=3
        )
        ...
        if self.expression == CommanderExpression.PLEASED:
            ...
            pygame.draw.arc(
                screen, AIR_FORCE_BLUE, (self.x - 18, eye_y - 5, 12, 10), 3.14, 6.28, 2
            )
            pygame.draw.arc(
                screen, AIR_FORCE_BLUE, (self.x + 6, eye_y - 5, 12, 10), 3.14, 6.28, 2
            )
        ...
        if self.expression == CommanderExpression.PLEASED:
            pygame.draw.arc(
                screen,
                AIR_FORCE_BLUE,
                (self.x - 12, mouth_y - 8, 24, 16),
                3.14,
                6.28,
                2,
            )
        elif self.expression in (
            CommanderExpression.ANGRY,
            CommanderExpression.FURIOUS,
        ):
            pygame.draw.arc(
                screen, AIR_FORCE_BLUE, (self.x - 12, mouth_y, 24, 12), 0, 3.14, 2
            )
        ...
        for dx in [-self.WIDTH // 2 + 15, self.WIDTH // 2 - 15]:
            pygame.draw.rect(
                screen, (255, 215, 0), (self.x + dx - 8, body_rect.y + 10, 16, 4)
            )
            pygame.draw.rect(
                screen, (255, 215, 0), (self.x + dx - 8, body_rect.y + 18, 16, 4)
            )

        # Arms crossed (simple)
        pygame.draw.ellipse(screen, AIR_FORCE_BLUE, (self.x - 40, self.y - 20, 80, 40))
