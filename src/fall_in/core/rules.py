...
from fall_in.config import (
    NUM_PLAYERS, CARDS_PER_PLAYER, NUM_ROWS, GAME_OVER_SCORE
)
...
class GameRules:
    ...
    def all_players_selected(self) -> bool:
        ...
        return all(p.selected_card is not None for p in self.players if not p.is_eliminated)
    
    def get_cards_in_play_order(self) -> list[tuple[Player, Card]]:
        """
        Get selected cards in ascending order (placement order).
        
        Returns:
            List of (player, card) tuples sorted by card number
        """
        selections = []
        for player in self.players:
        ...
        selections.sort(key=lambda x: x[1].number)
        return selections
